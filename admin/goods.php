...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	//首页推荐缩略图 qxfz.cn
    if (isset($_FILES['goods_thumb2']) && $_FILES['goods_thumb2']['tmp_name'] != '' &&
        isset($_FILES['goods_thumb2']['tmp_name']) &&$_FILES['goods_thumb2']['tmp_name'] != 'none')
    {
		$goods_thumb2 = $image->upload_image($_FILES['goods_thumb2'],'index_gimg');
		if ($goods_thumb2 === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
    }
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb,goods_thumb2, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb','$goods_thumb2', '$original_img', ".
    ...
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb,goods_thumb2, goods_img, original_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
		...
		if ($proc_thumb && $goods_thumb2 && $row['goods_thumb2'] && !goods_parse_url($row['goods_thumb2']))
        {
            @unlink(ROOT_PATH . $row['goods_thumb2']);
        }
