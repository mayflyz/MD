...
class cls_template
{
    ...
    */
        $tmp = $this;
        return preg_replace_callback("/{([^\}\{\n]*)}/", function ($r) use ($tmp){return $tmp->select($r[1]);}, $source);
    }
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace_callback("/(\'\\$[^,]+)/", function ($r){stripslashes(trim($r[1],'\''));},var_export($t, true)) . ";\n";
    ...
    function get_val($val)
    {
        if (strrpos($val, '[') !== false)
        {
            ...
            $val = preg_replace_callback("/\[([^\[\]]*)\]/is", function ($r){'.'.str_replace('$','$',$r[1]);}, $val);
        }
